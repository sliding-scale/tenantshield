...
/** Cream page background to match mobile design; `stone-50` on lg for card shell */
const pageBgMobile = "bg-cream-page"

export default function ImpactScorePage() {
  const router = useRouter()
  const { clerkUser, role, isLoading: userLoading } = useCurrentUser()
  const impact = useQuery(api.onboarding.queries.getImpactScore, clerkUser ? {} : "skip")
  const onboardingStatus = useQuery(api.onboarding.queries.onboardingStatus, clerkUser ? {} : "skip")
  ...
  if (!impact) {
    return (
      <main
        className={`min-h-[100dvh] ${pageBgMobile} md:min-h-[calc(100vh-4rem)] dark:bg-background`}
      >
        <div className="mx-auto flex w-full max-w-3xl flex-col justify-center px-5 py-12 sm:px-6 lg:max-w-4xl lg:px-8 lg:py-20">
          ...
          <div className="mt-10">
            <Button asChild className="h-12 min-h-11 rounded-xl px-8 text-base font-semibold">
  ...
  return (
    <main
      className={`w-full ${pageBgMobile} dark:bg-background max-lg:min-h-[100dvh] lg:flex lg:h-[calc(100dvh-4rem)] lg:max-h-[calc(100dvh-4rem)] lg:min-h-0 lg:flex-col lg:overflow-hidden lg:bg-stone-50`}
    >
      {/* Mobile: scrollable single column, mock-aligned */}
      <div className="mx-auto w-full max-w-md px-5 pb-14 pt-8 sm:px-6 lg:hidden">
        <div className={`inline-flex rounded-full px-4 py-2 text-[0.7rem] font-semibold uppercase tracking-[0.2em] text-white ${style}`}>
          {label}
        </div>
        ...
        <p className="font-heading text-2xl font-semibold leading-snug text-foreground sm:text-[1.65rem] sm:leading-snug">
          Renters like you typically recover 3–5x more than the cost of the service, in one letter.
        </p>
        ...
        <ul className="mt-8 space-y-5">
          {featureItems.map((item) => (
            <li key={item} className="flex items-start gap-4">
              ...
              <span className="pt-0.5 text-lg leading-snug text-foreground">{item}</span>
            </li>
          ))}
        </ul>
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-3xl border border-border bg-background shadow-sm ring-1 ring-black/5 dark:ring-white/10">
          <div className="flex min-h-0 flex-1 flex-col lg:grid lg:grid-cols-[minmax(240px,36%)_1fr] lg:grid-rows-1 lg:gap-0 xl:grid-cols-[minmax(260px,34%)_1fr]">
            <div className="flex min-h-0 flex-1 flex-col justify-center border-b border-border bg-muted/20 px-5 py-5 sm:px-6 sm:py-6 lg:border-b-0 lg:border-r lg:py-5 xl:px-8">
              <div className={`w-fit rounded-full px-3 py-1.5 text-[0.65rem] font-semibold uppercase tracking-[0.18em] text-white sm:px-4 sm:py-2 sm:text-xs sm:tracking-[0.2em] ${style}`}>
                {label}
              </div>
