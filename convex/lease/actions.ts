...
${args.leaseText}`,
    ...
    const baseOptimizedQuery =
      ...
      `tenant lease law review  ${args.state} USA`;
...
- Return JSON only, matching schema exactly.`,
    ...
    const textToEmbed = [
      `LEASE REVIEW: ${generatedData.leaseReview} (${args.state})`,
    ...
    const chunkPayloads = [
      {
        chunkType: "full_lease_review",
        chunkText: textToEmbed,
      },
      {
        chunkType: "document_summary",
        chunkText: generatedData.documentSummary,
      },
      {
        chunkType: "red_flags",
        chunkText: generatedData.redFlags
          .map((rf, index) => `${index + 1}. Quote: ${rf.quote}\nProblem: ${rf.problem}`)
          .join("\n\n"),
      },
      {
      ...
      },
      {
        ...
        chunkText: generatedData.tenantFriendlyClauses
          .map((tf, index) => `${index + 1}. Quote: ${tf.quote}\nExplanation: ${tf.explanation}`)
          .join("\n\n"),
      },
      {
