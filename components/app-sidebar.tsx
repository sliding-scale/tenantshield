...
import { ADMIN_NAV_ITEMS, APP_NAV_ITEMS, MOBILE_MORE_NAV_ITEMS, type AppNavItem } from '@/lib/nav/items';

const SIDEBAR_EXCLUDED_HREFS = new Set(['/billing', '/profile']);

const moreNavItems: AppNavItem[] = MOBILE_MORE_NAV_ITEMS.filter(
  (item) => !APP_NAV_ITEMS.some((main) => main.href === item.href) && !SIDEBAR_EXCLUDED_HREFS.has(item.href),
);
...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  const navItems = (isAdmin ? ADMIN_NAV_ITEMS : APP_NAV_ITEMS).filter((item) => !SIDEBAR_EXCLUDED_HREFS.has(item.href));
